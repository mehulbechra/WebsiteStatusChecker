...
workers.gatherAllChecks = () => {
  // Get all checks
  _data.list('checks', (listErr, checks) => {
...
workers.performCheck = (originalCheckData) => {
  ...
  const { path } = parsedUrl; // using path (not pathname) as the query string is required
...
// Append log data for each check that occurs
workers.log = (originalCheckData, checkOutcome, state, alertWanted, timeOfCheck) => {
  // Form the log data
  const logData = {
      ...
      logs.forEach((logName) => {
        ...
        _logs.compress(logId, newFileId, (compressErr) => {
          if (!compressErr) {
            ...
            debug('Error compressing one of the log file', compressErr);
          }
        });
      });
    } else {
