...
handlers.index = (data, callback) => {
  ...
  if (data.method === 'get') {
    ...
    const templateData = {
      ...
      'head.description': 'Free monitoring for http/https sites. A sms will be sent upon status change.',
      'body.class': 'index',
    };
...
handlers.sessionCreate = (data, callback) => {
  ...
  if (data.method === 'get') {
    ...
    // Read index template as string
    helpers.getTemplate('sessionCreate', templateData, (sessCreateErr, sessCreateStr) => {
      if (!sessCreateErr && sessCreateStr) {
        // Add the universal header and footer
        helpers.addUniversalTemplates(sessCreateStr, templateData, (err, str) => {
          if (!err && str) {
            callback(200, str, 'html');
          } else {
            callback(500, undefined, 'html');
...
handlers.sessionDeleted = (data, callback) => {
  ...
  if (data.method === 'get') {
    ...
    // Read index template as string
    helpers.getTemplate('sessionDeleted', templateData, (sessCreateErr, sessCreateStr) => {
      if (!sessCreateErr && sessCreateStr) {
        // Add the universal header and footer
        helpers.addUniversalTemplates(sessCreateStr, templateData, (err, str) => {
          if (!err && str) {
...
handlers.accountEdit = (data, callback) => {
  ...
  if (data.method === 'get') {
    ...
    // Read index template as string
    helpers.getTemplate('accountEdit', templateData, (sessCreateErr, sessCreateStr) => {
      if (!sessCreateErr && sessCreateStr) {
        // Add the universal header and footer
        helpers.addUniversalTemplates(sessCreateStr, templateData, (err, str) => {
          if (!err && str) {
...
handlers.accountDeleted = (data, callback) => {
  ...
  if (data.method === 'get') {
    ...
    // Read index template as string
    helpers.getTemplate('accountDeleted', templateData, (sessCreateErr, sessCreateStr) => {
      if (!sessCreateErr && sessCreateStr) {
        // Add the universal header and footer
        helpers.addUniversalTemplates(sessCreateStr, templateData, (err, str) => {
          if (!err && str) {
...
handlers.checksCreate = (data, callback) => {
  ...
  if (data.method === 'get') {
    ...
    // Read index template as string
    helpers.getTemplate('checksCreate', templateData, (sessCreateErr, sessCreateStr) => {
      if (!sessCreateErr && sessCreateStr) {
        // Add the universal header and footer
        helpers.addUniversalTemplates(sessCreateStr, templateData, (err, str) => {
          if (!err && str) {
...
handlers.checksList = (data, callback) => {
  ...
  if (data.method === 'get') {
    ...
    // Read index template as string
    helpers.getTemplate('checksList', templateData, (sessCreateErr, sessCreateStr) => {
      if (!sessCreateErr && sessCreateStr) {
        // Add the universal header and footer
        helpers.addUniversalTemplates(sessCreateStr, templateData, (err, str) => {
          if (!err && str) {
...
handlers.checksEdit = (data, callback) => {
  ...
  if (data.method === 'get') {
    ...
    // Read index template as string
    helpers.getTemplate('checksEdit', templateData, (sessCreateErr, sessCreateStr) => {
      if (!sessCreateErr && sessCreateStr) {
        // Add the universal header and footer
        helpers.addUniversalTemplates(sessCreateStr, templateData, (err, str) => {
          if (!err && str) {
  ...
  if (phone && password) {
    ...
    _data.read('users', phone, (err, userData) => {
      ...
      if (!err && userData) {
        ...
        const hashedPassword = helpers.hash(password);
        _performance.mark('Password hasing complete');
        ...
        _data.read('users', userPhone, (userReadErr, userData) => {
          if (!userReadErr && userData) {
            ...
            if (userChecks.length < config.maxChecks) {
              // Verify that the given url has DNS entries (and therfore can resolve)
              ...
              dns.resolve(hostName, (dnsErr, records) => {
                if (!dnsErr && records) {
                  ...
                  callback(400, { Error: 'The hostname of the URL entered didnot resolve any DNS entries' });
                }
              });
            } else {
